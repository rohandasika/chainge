...
In the end, the app was deployed using Truffle and the Matic Vigil `wss://` endpoint ([Deployed contract](https://mumbai.polygonscan.com/address/0x17FAd95EB36EEC17c1E6DDF8e1CCD3C444B99071)). There was a lot of online discussion in the Truffle forums about using only the Infura `wss://` endpoints (instead of `http://`), but unfortunately, Infura _only_ has a `http://` for the Polygon Mumbai testnet.
